...
NAME=$(grep -o '"name": "[^"]*"' package.json | grep -o '\:[ ]*"[^"]*' | sed 's/\:[ ]*"//g')

echo "This is an internal Dojo release script!"
echo -n "Press 'y' to create a new $NAME release from branch $BRANCH"
if [ "$VERSION" == "" ]; then
	echo "."
else
...
# $PRE_VERSION is the next pre-release version that will be set on the original branch after tagging
# $MAKE_BRANCH is the name of the new minor release branch that should be created (if this is not a patch release)
# $BRANCH_VERSION is the pre-release version that will be set on the minor release branch
